...
type PublicKey struct {
	...
	Prec uint16   // P: binary precision in bits
	N    uint16   // n: decimal precision (digits)
}
...
func (pk *PublicKey) MarshalBinary() ([]byte, error) {
	...
	binary.BigEndian.PutUint16(buf[0:2], pk.Prec)
	binary.BigEndian.PutUint16(buf[2:4], pk.N)
...
func computeCommonDenominator(p, n uint16) *big.Int {
	twoPowP := new(big.Int).Lsh(big.NewInt(1), uint(p))
...
func liftToCommonDomain(f *big.Float, d *big.Int, prec uint16) (*big.Int, error) {
	if f.Sign() < 0 || f.Cmp(big.NewFloat(1).SetPrec(uint(f.Prec()))) >= 0 {
		return nil, errors.New("value not in [0,1)")
	}

	dFloat := new(big.Float).SetPrec(uint(prec)).SetInt(d)
	scaled := new(big.Float).SetPrec(uint(prec)).Mul(f, dFloat)
